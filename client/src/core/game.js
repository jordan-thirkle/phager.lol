...
window.addEventListener('error', (e) => {
  console.error('🔥 CLIENT CRASH:', e.message, 'at', e.filename, ':', e.lineno);
});
...
// Using window.LS directly to avoid redeclaration issues across scripts
...
var AppState = {
  ...
  clientSeq: 1, lastProcessedSeq: 0, pendingInputs: [],
  myLocalBlobs: [] // For prediction reconciliation
};
...
function checkLibraries(onReady) {
  ...
  const check = setInterval(() => {
    attempts++;
    const hasPC = typeof pc !== 'undefined';
    const hasMP = typeof MessagePack !== 'undefined';
    const hasIO = typeof io !== 'undefined';
    
    if (hasPC && hasMP && hasIO) {
      clearInterval(check);
      console.log('✅ ALL LIBRARIES READY (Attempt '+attempts+')');
      onReady();
    } else if (attempts > 100) { // 10 seconds
      clearInterval(check);
      console.warn('⚠️ Libraries timed out. Missing:', { pc:hasPC, msgpack:hasMP, io:hasIO });
      onReady(); // Try anyway
    }
  }, 100);
}
...
window.startGame = function() {
  if (AppState.gameStarting || AppState.gameActive) return;
  ...
  checkLibraries(() => {
    ...
    window.LS.set('name', AppState.myName);
...
function loadStartStats() {
  ...
  const savedName = window.LS.get('name','');
...
function initPC() {
  ...
  AppState.app.on('update', dt => { 
    ...
    if(AppState.gameActive && AppState.app) gameLoop(dt); 
  });
  AppState.app.start();
}
...
function makeMat(color, intensity=2) {
  ...
  /* 
  mat.onUpdateShader = (options) => {
    if (options && pc.FRESNEL_SCHLICK) options.fresnelModel = pc.FRESNEL_SCHLICK;
    return options;
  };
  */
...
function gameLoop(dt) {
  ...
  if (AppState.sendTimer >= 0.05) { 
    ...
    AppState.pendingInputs.push(pkt); // Store for reconciliation
  ...
  // --- CLIENT-SIDE PREDICTION (Rule #3 Remediation) ---
  const me = AppState.gameState.players.find(p => p && p.id === AppState.myId);
  if (me && me.blobs) {
    me.blobs.forEach(b => {
...
function connectSocket() {
  ...
  AppState.socket.on('world_state', buf => {
    ...
    if (meServer && meServer.lastSeq !== undefined) {
        // SERVER RECONCILIATION (Rule #3 Remediation)
        ...
        // Find current local state of 'me'
        const meLocal = AppState.gameState.players.find(p => p && p.id === AppState.myId);
        if (meLocal && meLocal.blobs) {
            // Re-apply unacknowledged inputs to the server's authoritative position
            meServer.blobs.forEach((sb, i) => {
                const lb = meLocal.blobs[i];
                if (lb) {
                    // SOFT RECONCILIATION: Instead of hard snap, blend toward target
                    const targetX = sb.x;
                    const targetZ = sb.z;
                    
                    // Predict future position from pending inputs
                    let predX = targetX, predZ = targetZ;
                    ...
                    // Blend local toward predicted
                    const blend = 0.4; // Soften the snap
  ...
  AppState.socket.on('feedbackDecoyHit', ({x, z}) => {
    ...
    if (window.Audio) Audio.virusHit(); // Re-use virus sound for penalty
    AppState.shakeAmt = 20;
    AppState.aberrationAmt = 0.4;
  });

  AppState.socket.on('decoyPopped', ({x, z, color}) => {
    if (window.Particles) Particles.virusExplosion(x, 10, z, color);
  });
