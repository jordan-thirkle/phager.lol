...
export const drawers = {
  ...
  dots(ctx, color) {
    ctx.fillStyle = lighter(color, 120); ctx.globalAlpha = 0.5;
    for (let i = 0; i < 12; i++) {
  ...
  void(ctx, color) {
    ...
    radii.forEach((r, i) => {
      ...
      ctx.strokeStyle = color; ctx.globalAlpha = 0.7 - (i * 0.1); ctx.lineWidth = 3; ctx.stroke();
    });
    ctx.fillStyle = '#fff'; ctx.globalAlpha = 1;
    for (let i = 0; i < 80; i++) ctx.fillRect(Math.random() * SIZE, Math.random() * SIZE, 1, 1);
  }
};
...
export const Skins = {
  ...
  patternIndexForId(id) {
    let h = 0; for (let i = 0; i < id.length; i++) h = (h * 31 + id.charCodeAt(i)) >>> 0;
    return h % PATTERNS.length;
  },

  getTexture(app, id, color) {
    const key = `${id}_${color}`;
    if (cache[key]) return cache[key];
    const cv = document.createElement('canvas'); cv.width = SIZE; cv.height = SIZE;
    const ctx = cv.getContext('2d', { willReadFrequently: true });
    const patName = PATTERNS[this.patternIndexForId(id)];
    drawBase(ctx, color);
