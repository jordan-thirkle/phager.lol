const BattleRoyale = {
  ...
  checkWinCondition(gs) {
    ...
    // TIEBREAKER: If zone is at minimum and multiple players remain, or only 1 remains
    ...
    } else if (this.zone.radius <= 50 && playersAlive.length > 1) {
        // Tiebreaker Logic: Sort by Mass > Kills > Join Order
        const sorted = playersAlive.sort((a,b) => {
            const aMass = a.blobs.reduce((s,bl)=>s+bl.mass,0);
            const bMass = b.blobs.reduce((s,bl)=>s+bl.mass,0);
            if (Math.abs(aMass - bMass) > 1) return bMass - aMass;
            return (b.kills || 0) - (a.kills || 0);
        });
        const winner = sorted[0];
        this.placements.unshift({ name: winner.name, kills: winner.kills || 0, massAtDeath: Math.floor(winner.blobs.reduce((s,b)=>s+b.mass,0)) });
        return { winner: winner, type: 'BATTLE_ROYALE_VICTORY', placements: this.placements, tiebreak: true };
    }
    return null;
  }
};
