...
class GameRoom {
  ...
  buildSpatialHash() {
    this.grid.clear();
    const foodsValues = Object.values(this.foods);
    for (let i = 0; i < foodsValues.length; i++) {
      const f = foodsValues[i];
    ...
    const virusesValues = Object.values(this.viruses);
    for (let i = 0; i < virusesValues.length; i++) {
      const v = virusesValues[i];
    ...
    for (let j = 0; j < playersValues.length; j++) {
      const p = playersValues[j];
      ...
      for (let i = 0; i < p.blobs.length; i++) {
        ...
        this.grid.get(key).blobs.push({ pid: p.id, idx: i, blob: b });
      }
    }
